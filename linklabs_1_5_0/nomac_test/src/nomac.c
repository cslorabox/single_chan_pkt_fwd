...
int rx_mode_single(char *buf, bool discard)
{
    static uint8_t rx_buf[RX_BUF_LEN];
    uint8_t rx_len;
    int c;
...
int rx_mode_cont(uint32_t receive_time_ms, uint8_t has_freq_err,
			 char *buf)
{
    static uint8_t rx_buf[RX_BUF_LEN];
    uint8_t rx_len;
...
void ll_setup(char *tty, uint8_t sf, uint32_t freq) {
  ll_tty_open(tty, 115200);
  print_ll_firmware_type();
  print_ll_hardware_type();
  print_ll_version();
  int32_t i32_ret;
  i32_ret = ll_spreading_factor_set(sf);
...
int nomac_main (int argc, char * const argv[])
{
  int p;
  for (p=0; p< argc; p++)
    printf("%d : %s\r\n", p, argv[p]);
    ...
    switch (mode)
    {
        ...
        case MODE_ECHO_TX_PAYLOAD:
            ...
            break;
        case MODE_RX:
            rx_mode_single(NULL, 1);
            break;
        case MODE_RX_CONT:
	  rx_mode_cont(receive_time_ms, rx_cont_freq_err_requested, NULL);
            break;
        case MODE_TX:
